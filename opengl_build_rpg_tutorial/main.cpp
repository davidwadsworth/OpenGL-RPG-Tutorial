...
Rect flesh_dest = { 5.0f, 5.0f, 64.0f, 64.0f };
Rect flesh_dest2 = { 800.0f - 69.0f, 600.0f - 69.0f, 64.0f, 64.0f };
...
int main()
{
    ...
    assert(tile_map->children_size() == 1);
    assert(tile_map->component_size() == 1);
    assert(player->component_size() == 1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (auto i = 0; i < total_grass; ++i)
        {
            ...
            renderer.draw(SRC, grass_dest, c_tmap_material);
        }

        // draw flesh children
        renderer.draw(SRC, flesh_dest, c_pla_material);
        renderer.draw(SRC, flesh_dest2, c_pla_material);
