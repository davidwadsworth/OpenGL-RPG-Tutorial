...
Component::Rectangle* add_component_rect(Entity* entity, Entity* gamestate, nlohmann::json info_json)
{
		...
		if (collider_name == "circle")
		{
			...
			if (has_action)
			{
				auto ccgjkaps_circle = entity->push_back_component
					...
					(Rect(x, y, w, h), radius, glm::vec2(center_x, center_y));
		...
		else if (collider_name == "boundary")
		{
			...
			if (has_action)
			{
				auto ccgjkaps_boundary = entity->push_back_component
					<Component::Collider::GJK::Action::Physics::Smooth::Boundary>(Rect(x, y, w, h), arr_points);
				ccgjkaps_boundary->action = info_json["action"];
				rect = ccgjkaps_boundary;
			}
			else
				rect = entity->push_back_component
					<Component::Collider::GJK::Physics::Boundary::Smooth>(Rect(x, y, w, h), arr_points);
		}
		else if (collider_name == "polygon")
		{
			...
			if (has_action)
			{
				auto ccgjkaps_polygon = entity->push_back_component
					<Component::Collider::GJK::Action::Physics::Smooth::Polygon>(Rect(x, y, w, h), vec2_points);
	...
	else
	{
		if (has_action)
		{
			auto cr_action = entity->push_back_component<Component::ARect>(Rect(x, y, w, h));
			cr_action->action = info_json["action"];
			rect = cr_action;
		}
		else
			rect = entity->push_back_component<Component::Rectangle>(Rect(x, y, w, h));
	}
